...
const play = (connection, url, msg, lang, self) => {
  try {
    const stream = ytdl(url, { filter : 'audioonly' })
    self.loop[msg.guild.id] = {}
    self.playing[msg.guild.id] = url
...
module.exports = class extends Command {
  ...
  async run(msg, settings, lang) {
    ...
    this.loop[msg.guild.id] = {} // need initialize!
    if (args[1] === 'join') {
      ...
      msg.member.voiceChannel.join()
        .then(async connection => {
          if (msg.guild.voiceConnection && msg.guild.voiceConnection.dispatcher && !msg.guild.voiceConnection.dispatcher.destroyed) {
            if (this.loop[msg.guild.id].enabled) return msg.channel.send(lang.music.cantadd_loop_enabled)
            if (!this.queue[msg.guild.id]) this.queue[msg.guild.id] = []
            this.queue[msg.guild.id].push(args[2])
            msg.channel.send(f(lang.music.queue_added, args[2]))
            if (keyword) msg.channel.send('Search keyword: ' + keyword)
          } else {
            play(connection, args[2], msg, lang, self)
            msg.channel.send(f(lang.music.playing, args[2]))
            if (keyword) msg.channel.send('Search keyword: ' + keyword)
          }
          if (msg.deletable) msg.delete()
          const endHandler = async () => {
            const q = this.queue[msg.guild.id]
            if (q && (q ? q.length : false) && !this.loop[msg.guild.id].enabled) { // if queue is *not* empty and not enabled loop
              play(connection, q[0], msg, lang, self)
              msg.channel.send(f(lang.music.playing_queue, q[0]))
              this.queue[msg.guild.id] = this.queue[msg.guild.id].slice(1)
            } else { // if queue is empty or enabled looping
              if (!this.loop[msg.guild.id].enabled && !this.loop[msg.guild.id].every) { // loop is disabled
                ...
                if (this.loop[msg.guild.id].every) {
                  const seconds = parseInt(this.loop[msg.guild.id].every.replace(/\D{1,}/gm, '')) * 60
                  setTimeout(() => {
                    play(connection, args[2], msg, lang, self)
                  }, seconds * 1000)
                } else {
      ...
      if (!this.loop[msg.guild.id].enabled && msg.guild.voiceConnection && msg.guild.voiceConnection.dispatcher && !msg.guild.voiceConnection.dispatcher.destroyed) {
      ...
      Object.assign({[msg.guild.id]: []}, this.queue)
      //queue.push({[msg.guild.id]: []})
      if (args[2] === 'disable') {
        this.loop[msg.guild.id].every = null
        this.loop[msg.guild.id].enabled = false
        msg.channel.send(lang.music.disabled_looping)
      } else {
        if (args[2] === 'every') {
          if (args[3] === 'clear') return this.loop[msg.guild.id].every = null
          if (!/\d{1,}m/gm.test(args[3])) return msg.channel.send(lang.invalid_args)
          this.loop[msg.guild.id].enabled = true
          this.loop[msg.guild.id].every = args[3]
          msg.channel.send(f(lang.music.loop_every, args[3]))
        } else {
          this.loop[msg.guild.id].enabled = this.loop[msg.guild.id].enabled ? false : true
          msg.channel.send(f(lang.music.one_music_loop, this.loop[msg.guild.id].enabled ? lang.music.enabled : lang.music.disabled))
        }
      }
      ...
      msg.channel.send(f(lang.music.queue_count, this.queue.length))
      if (args[2] === 'clear') {
        this.queue[msg.guild.id] = []
      ...
      const queues = this.queue[msg.guild.id] ? this.queue[msg.guild.id].map((e, i) => `${i}: ${e}`) : []
      if (queues.length) msg.channel.send(queues.join('\n'))
    } else if (args[1] === 'status') {
      if (msg.guild.voiceConnection && msg.guild.voiceConnection.dispatcher && !msg.guild.voiceConnection.dispatcher.destroyed && this.playing[msg.guild.id]) msg.channel.send(f(lang.music.now_playing, this.playing[msg.guild.id]))
      else msg.channel.send(lang.music.not_playing)
